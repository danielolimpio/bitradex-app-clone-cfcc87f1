...
const Footer = () => {
  ...
  return <footer className="bg-bitradx-darker border-t border-border">
        ...
        <div className="py-16 grid lg:grid-cols-6 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              <h4 className="text-lg text-foreground font-normal">Stay Updated</h4>
              <div className="flex gap-2">
                <Input placeholder="Enter your email" className="bg-background/50 border-border" />
          ...
          {footerSections.map((section, index) => <div key={index} className="space-y-4">
              ...
              <ul className="space-y-3">
                {section.links.map((link, linkIndex) => <li key={linkIndex}>
                    {link.href.startsWith('/') ? <Link to={link.href} className="text-muted-foreground hover:text-primary transition-colors duration-200 text-sm">
                        {link.text}
                      </Link> : <a href={link.href} className="text-muted-foreground hover:text-primary transition-colors duration-200 text-sm">
                        {link.text}
                      </a>}
                  </li>)}
              </ul>
        ...
        <div className="py-8 border-t border-border">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            ...
            <div className="flex items-center space-x-4">
              {socialLinks.map((social, index) => <a key={index} href={social.href} aria-label={social.label} className="w-10 h-10 bg-gradient-card border border-border rounded-lg flex items-center justify-center hover:bg-primary hover:border-primary transition-all duration-300 group">
